...
const products: ProductosServices = new ProductosServices();
const { productos } = storeToRefs(myStore());
...
onBeforeMount(async () => {
  ...
  await products.initProductos(token);
  productos.value = products.getProductos().productos!;
